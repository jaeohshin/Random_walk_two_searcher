...
* July 23rd- the position of target is varying
**/
...
int main(){
	...
	int L=100; //lattice coordinate= [-L/2, +L/2]
	
	double koff= 0.5 ; // This is the dissociation rate from the target site.
	...
	int find1, find2 ;
	int m1 ;  //target position m1=[0, L/2]
...
//		fprintf(tFile, "%d\n", i );
	...
	if (n1==n2) {
		...
		if (r< kd) {  ///dissociate
			...
			else {   // the complex is not at the edge of the lattice
				r=genrand64_real2();
				if (r< 0.5)	{n1=n1-1;
							}
